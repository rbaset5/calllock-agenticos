...
- Local presence caller ID now resolves correctly from metro city names (e.g., "Phoenix" → AZ number, "Detroit" → MI number) instead of always falling back to the Texas number
...
- Timezone clock strip in the dialer showing local time for each target market (TX, FL, IL, MI, AZ)
- Lazy Twilio initialization — voice SDK only loads on first user interaction, reducing page load overhead
