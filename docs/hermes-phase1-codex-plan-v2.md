...
```bash
cd harness && PYTHONPATH=src python -c "from harness.hermes_adapter import BLOCKED_TOOLSETS; print('Adapter imports OK')"
```
...
```bash
cd harness && PYTHONPATH=src python -m pytest tests/test_hermes_adapter.py -x -q
```
