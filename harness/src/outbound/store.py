    ...
    for record in records:
        duplicate = next(
            (
                ...
                if row["tenant_id"] == record.get("tenant_id", OUTBOUND_TENANT_ID)
                and row["phone_normalized"] == record["phone_normalized"]
            ),
            None,
        )
        ...
        stored = {
            ...
            "twenty_company_id": record.get("twenty_company_id"),
        }
...
def insert_prospect_signal(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    record = {
        ...
        "signal_tier": int(payload.get("signal_tier", 1)),
...
def insert_call_test(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    record = {
        ...
        "tenant_id": payload.get("tenant_id", OUTBOUND_TENANT_ID),
        "prospect_id": payload["prospect_id"],
        "twilio_call_sid": payload.get("twilio_call_sid", str(uuid4())),
        "called_at": payload.get("called_at", _now_iso()),
        "day_of_week": payload.get("day_of_week", "monday"),
        "local_time": payload.get("local_time", "09:00"),
...
def insert_outbound_call(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    duplicate = next(
        (
            ...
            if row["tenant_id"] == payload.get("tenant_id", OUTBOUND_TENANT_ID)
            and row["twilio_call_sid"] == payload["twilio_call_sid"]
        ),
        None,
    )
    if duplicate is not None:
