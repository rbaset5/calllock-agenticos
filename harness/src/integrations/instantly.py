...
class InstantlyClient:
    ...
    def __init__(
        ...
        self.base_url = (
            base_url or os.environ.get("INSTANTLY_BASE_URL", DEFAULT_BASE_URL)
        ).rstrip("/")
    ...
    async def _request(
        ...
        full_url = f"{url}?{parse.urlencode(all_params, doseq=True)}"
        ...
        req = request.Request(full_url, data=data, method=method, headers=headers)
        ...
        try:
            ...
            logger.error(
                "instantly_api_error",
                extra={"status": exc.code, "path": path, "body": body[:500]},
            )
            raise InstantlyError(
                f"Instantly API {method} {path} failed: {exc.code} {body[:200]}"
            ) from exc
    ...
    async def send(
        ...
        if not skip_guardrails:
            ...
            bounce_rate = reputation.get("bounce_rate", 0)
            complaint_rate = reputation.get("complaint_rate", 0)
            ...
            send_log = await self.get_send_log(account_id)
    ...
    def _check_rate_limit(self, account_id: str) -> None:
        ...
        if account_id in self._send_log:
            self._send_log[account_id] = [
                sent_at for sent_at in self._send_log[account_id] if sent_at > one_hour_ago
            ]
